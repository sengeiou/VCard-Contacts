...
public class RawContactReadOnlyEditorView extends BaseRawContactEditorView
    ...
    public void setState(RawContactDelta state, AccountType type, ViewIdGenerator vig,
        ...
        if (accountInfo == null) {
            ...
            mAccountHeaderNameTextView.setVisibility(GONE);
        } else {
            if (accountInfo.first == null) {
                mAccountHeaderNameTextView.setVisibility(GONE);
            } else {
                mAccountHeaderNameTextView.setVisibility(VISIBLE);
        ...
        if (type.getEditContactActivityClassName() != null) {
            mEditExternallyButton.setVisibility(VISIBLE);
        } else {
            mEditExternallyButton.setVisibility(GONE);
        }

        final Resources res = getContext().getResources();
        // Phones
        final ArrayList<ValuesDelta> phones = state.getMimeEntries(Phone.CONTENT_ITEM_TYPE);
        final Drawable phoneDrawable = getResources().getDrawable(R.mipmap.ic_phone_24dp);
        ...
        final Drawable emailDrawable = getResources().getDrawable(R.mipmap.ic_email_24dp);
        ...
        if (mGeneral.getChildCount() > 0) {
            mGeneral.setVisibility(VISIBLE);
        } else {
            mGeneral.setVisibility(GONE);
        }
    }
    ...
    private void bindData(Drawable icon, String iconContentDescription, CharSequence data,
        ...
        if (forceLTR) {
            dataView.setTextDirection(TEXT_DIRECTION_LTR);
        }
